...
public class LogInDaoImpl implements LogInDao {
    ...
    public String logInUser(LogInDTO logInDTO) {
        Optional<Admin> admin = adminRepo.findByUsername(logInDTO.getUserName());

        Optional<Customer> customer = customerRepo.findByUsername(logInDTO.getUserName());

        Optional<Driver> driver = driverRepo.findByUsername(logInDTO.getUserName());

        if (admin.isPresent()) {
            return logInAdmin(admin.get(), logInDTO);
        } else if (customer.isPresent()) {
            return logInCustomer(customer.get(), logInDTO);
        } else if (driver.isPresent()) {
            return logInDriver(driver.get(), logInDTO);
        } else {
