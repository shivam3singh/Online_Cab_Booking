...
public class AdminDaoImpl implements AdminDao{
    ...
    public Admin getAdmin(String uniqueKey) {
        ...
        return adminRepo.findById(userSession.getUserId()).orElseThrow(() -> new UserException("You are not an admin"));
    }
    ...
    public Admin updateAdmin(String uniqueKey, Admin admin) {
        ...
        if (customerUsernames.contains(admin.getUsername()) || driverUserNames.contains(admin.getUsername())) throw new UserException("Username already exists.");

        List<String> customerMobileNumbers = customerRepo.getAllMobileNumbers();
        List<String> driverMobileNumbers = driverRepo.getAllMobileNumbers();

        if (customerMobileNumbers.contains(admin.getMobileNumber()) || driverMobileNumbers.contains(admin.getMobileNumber())) throw new UserException("Mobile number already exists.");

        List<String> customerEmails = customerRepo.getAllEmails();
        List<String> driverEmails = driverRepo.getAllEmails();

        if (customerEmails.contains(admin.getEmail()) || driverEmails.contains(admin.getEmail())) throw new UserException("Email already exists.");
    ...
    public String deleteAdmin(String uniqueKey) {
        ...
        return "Admin with username " + existingAdmin.getUsername() + " deleted successfully.";
    }
